...
suite('TestRunParser Unit Tests', () => {
    ...
    test('Parse pass-only test', async () => {
        ...
        if (fs.existsSync(fixturesPath)) {
            console.log('DEBUG: ALL files in fixtures:', fs.readdirSync(fixturesPath));
        }
        console.log('DEBUG: Current working directory:', process.cwd());
        // Check if copyfiles ran
        ...
        assert.ok(
            testCase.assertions.every((a: any) => a.type === 'passed'),
            'All assertions should be passed',
        );
    ...
    test('Parse single-fail test (has FAILED and OK)', async () => {
        ...
        assert.strictEqual(
            testCase.status,
            'failed',
            'Test should be FAILED (because one assert failed)',
        );
        assert.strictEqual(
            testCase.assertions.length,
            2,
            'Should have 2 assertions (1 FAILED + 1 OK)',
        );

        // Check assertions
        const failedAssertion = testCase.assertions.find((a: any) => a.type === 'failed');
        const passedAssertion = testCase.assertions.find((a: any) => a.type === 'passed');
    ...
    test('Parse multi-fail test (multiple FAILED in same test)', async () => {
        ...
        assert.strictEqual(
            testCase.assertions.length,
            3,
            'Should have 3 assertions (2 FAILED + 1 OK)',
        );

        // Count assertion types
        const failedCount = testCase.assertions.filter((a: any) => a.type === 'failed').length;
        const passedCount = testCase.assertions.filter((a: any) => a.type === 'passed').length;
        ...
        // Check notes
        const notes = testCase.assertions.map((a: any) => a.note);
    ...
    test('Parse aborted test', async () => {
        ...
        const result = await TestRunParser.parseTestRun(logPath, [
        ...
        assert.strictEqual(
            testCase.assertions.length,
            3,
            'Should have 3 assertions (1 OK + 1 FAILED + 1 ABORTED)',
        );

        // Check assertion types
        const abortedAssertion = testCase.assertions.find((a: any) => a.type === 'aborted');
        assert.ok(abortedAssertion, 'Should have an aborted assertion');
        assert.strictEqual(
            abortedAssertion?.errorMessage,
            'ABORTED testcase (simulated unknownFunction)',
        );
    ...
    test('Parse stack traces correctly', async () => {
        ...
        assert.strictEqual(
            failedAssertion.stackTrace[0].line,
            79,
            'First stack trace line should be 79',
        );
